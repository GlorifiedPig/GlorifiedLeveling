...
function PANEL:Paint( w, h )
    ...
    local boxH = h / 2.5
    local boxW = w / 2.5
    local boxX = w - boxW - sideOffset
    local boxY = ( h / 2 ) - ( boxH / 2 )
    draw.RoundedBox( 12, boxX, boxY, boxW, boxH, Color( 51, 51, 51 ) )

    local progress = 4
    if progress < 10 then
        local progressW = ( boxW / 10 ) * progress
        render.SetScissorRect( boxX, 0, self:LocalToScreen( boxX ) + progressW, ScrH(), true )
        draw.RoundedBox( 12, boxX, boxY, boxW, boxH, Color( 89, 175, 63 ) )
    ...
    for i = 1, 9 do
        draw.RoundedBox( 0, boxX + ( ( boxW / 10 ) * i ), boxY, 1, boxH + 1, Color( 0, 0, 0, 155 ) )
    end
end
